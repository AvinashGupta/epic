...
# -min_width minimal width of the common regions
...
common_regions = function(gr_list, min_width = 0, min_coverage = floor(length(gr_list)/4), gap = 0.5) {
...
# -factors classifications which correspond to samples in ``gr``
...
#          if ``present`` is a single numeric value, it means the mean value should be larger than this.
...
# -genomic_features Genomic features that are used for annotating to regions in ``gr_list``, it should be a list of `GenomicRanges::GRanges` objects
#
# == details
